...
- **Product layout**: same `frontend/` + `backend/` split; `backend/src/db/` holds Drizzle schema, Hono handlers in `src/routes/`. Postgres runs in Docker on the product VM (`supabase-db` container, persistent volume) - not Cloud SQL. Self-hosted Supabase provides auth, realtime, storage. No third-party SaaS for app runtime.
